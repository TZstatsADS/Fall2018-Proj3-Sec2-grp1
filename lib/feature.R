...
feature <- function(LR_dir, HR_dir, n_points=1000){
  ...
  #       
  #       nb_points = c(imgLR_padded [row_idx[p],(col_idx[p]):(col_idx[p]+2),ch], 
  #                     imgLR_padded [row_idx[p]+1,col_idx[p],ch],
  #                     imgLR_padded [row_idx[p]+1,col_idx[p]+2,ch],
  #                     imgLR_padded [row_idx[p]+2,(col_idx[p]):(col_idx[p]+2),ch])
  #       
  #       featMat[(p + (i-1)*n_points),,ch] = nb_points - imgLR_padded[row_idx[p]+1, col_idx[p]+1, ch]
  #       
  #       ### step 2.2. save the corresponding 4 sub-pixels of imgHR in labMat
  #       sub_points = imgHR[(HR_ridx[p]-1):(HR_ridx[p]), (HR_cidx[p]-1):(HR_cidx[p]) ,ch]
  #       labMat[p + (i-1)*n_points,,ch] = sub_points - imgLR[HR_ridx[p]/2 +1, HR_cidx[p]/2 + 1, ch] 
  #     }
  #   }
