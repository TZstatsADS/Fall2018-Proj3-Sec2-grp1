...
```{r model_setup}
#depth, numTrees, minNode, shrinkage
#depth = seq(1,10, 3)
...
```{r loadlib}
source("../lib/train_yu.R")
source("../lib/test_yu.R")
```
...
source("../lib/cross_validation_yu.R")

# dim(feat_train) =  1500000,8,3
# dim(label_train) = 1500000,4,3
...
if(run.cv){
  ...
  for(k in 1:nrow(model_values)){
    ...
    err_cv[k,] <- cv.function(feat_train, label_train,  K, depth = 1, 
                              ...
                              shrinkage = model_values$shrinkage[k])  # K = 5
  }
